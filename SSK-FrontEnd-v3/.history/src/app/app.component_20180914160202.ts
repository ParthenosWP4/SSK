...
export class AppComponent implements  OnInit {
  ...
  browseItems: Array<string> = ['Find a scenario', 'Find a step', 'Find a resource'];
  ...
  ngOnInit() {
      ...
      $('.dropdown-item a.dropdown-toggle').on('click', function(e) {
        if (!$(this).next().hasClass('show')) {
          $(this).parents('.dropdown-item').first().find('.show').removeClass('show');
        }
        const $subMenu = $(this).next('.dropdown-item');
