...
def parseSVRL(svrl, tree):
    ...
    for fire in fired:
        location = getLocations(fire.attrs['location'], tree)
...
create_directory(reportsFolder)

scenariosFolder = os.getcwd() + "/" + reportsFolder + "/scenarios_temp"
create_directory(scenariosFolder)
try:
...
listScReports = get_files(scenariosFolder)
...
for report in listScReports:
    ...
    create_directory(reportFolder)
    try:
        svrl = loadReport(report)
        filePath = svrl.find('active-pattern')['document'][5:]
        tree = loadSource(filePath)
        diagnostic = parseSVRL(svrl, tree)
        try:
            writeCSV(diagnostic, report, reportFolder)
            #create ReadMe.txt
        except:
        ...
        if tree.getroot().get('type') == 'researchScenario':
            ...
            for step in steps:
                ...
                try:
                    ...
                    treeSt = loadSource(filePathSt)
                    stepDiag = parseSVRL(svrlSt, treeSt)
                    try:
                        writeCSV(stepDiag, input[3:], stepsFolder)
