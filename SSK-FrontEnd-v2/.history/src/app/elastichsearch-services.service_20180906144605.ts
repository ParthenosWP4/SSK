...
export class ElastichsearchServicesService {
  ...
  private tags = ['disciplines', 'object', 'techniques', 'activities', 'standards'] ;
...
getAllStandards() {
  this.loadTermsFromServer('standard').subscribe(
 /* this.loadStandardsFromServer().subscribe(
    ...
    error => {
      console.log('Error when load server');
    },
    () => {}
  );
}
