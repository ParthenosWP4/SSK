...
<!--
  ...
  <xsl:template match="tei:TEI[not(@xml:id)]" xmlns="http://www.tei-c.org/ns/1.0">
...
-->
  <xd:doc>
    <xd:desc>Template to split persname in forename and surname</xd:desc>
  </xd:doc>
  
  <xsl:template match="tei:persName" xmlns="http://www.tei-c.org/ns/1.0">
    
  <forename><xsl:value-of select="tokenize(., ' ')[1]"/></forename>
  <surname><xsl:value-of select="normalize-space(substring-after(., tokenize(., ' ')[1]))"/></surname>
  </xsl:template>
  ...
  <xsl:template match="text()[starts-with(., ' ') and ends-with(., ' ')]" xmlns="http://www.tei-c.org/ns/1.0">
        <xsl:choose>
  ...
  <xsl:template match="tei:TEI/@xml:id" xmlns="http://www.tei-c.org/ns/1.0">
