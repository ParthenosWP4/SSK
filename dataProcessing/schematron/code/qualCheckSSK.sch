...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:TEI">
      ...
      <sch:assert test="string(@type) = 'researchScenario' or 'researchStep'" role="error">TEI/@type
        should be either "researchScenario" or "researchStep". The current value of TEI/@type is
          "<sch:value-of select="@type"/>"</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    <!-- added to odd -->
    ...
    <sch:rule context="tei:titleStmt">
      <sch:assert test="tei:author" role="error">All scenarios and steps should have identified
        authors.</sch:assert>
      <sch:report test="not(tei:sponsor)" role="warning">The main sponsor of the SSK is the
        PARTHENOS project. All scenarios and steps should record this information.</sch:report>
    </sch:rule>
    <sch:rule context="tei:author">
      <sch:assert test="tei:persName and tei:affiliation" role="error">You need to supply the name
        and the affiliation of all authors.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    <!-- added to odd -->
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:availability">
      <sch:assert test="tei:licence/@target and tei:licence/tei:p" role="error">A licence should be
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:sourceDesc" role="warning">
      <sch:report test="child::*" role="information">sourceDesc in mandatory in TEI, so if you want
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:change">
      <sch:assert test="tei:date and tei:persName and tei:affiliation and tei:desc"
        role="information">When recording a revision, some precise metadata must be given : the date
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:div/tei:head">
      <sch:assert test="string-length(.) &gt; 9" role="error"> The title is too short: 10 characters
        minimum. Your title is <sch:value-of select="10 - string-length(.)"/> characters too
        short.</sch:assert>
      <sch:assert test="string-length(.) &lt; 101" role="error">The title is too long: 100
        characters maximum. Your title is <sch:value-of select="string-length(.) - 100"/> characters
        too long.</sch:assert>
      <sch:report test="normalize-space(.)" role="information">The title of a scenario should
        describe the main goal of the scenario.</sch:report>
    </sch:rule>
    
    <sch:rule context="tei:event/tei:head">
      <sch:assert test="string-length(.) &gt; 9" role="error"> The title is too short: 10 characters
        minimum </sch:assert>
      <sch:assert test="string-length(.) &lt; 100" role="error">The title is too long: 100
        characters maximum</sch:assert>
      <sch:assert test="matches(., '(^\w*(ing|ion|ment|ments) )|( \w*(ing|ion|ment|ments$))')"
        role="information">The title of a step should describe the action to perform, starting or
        ending with a gerund (or an infinitive), or a noun .</sch:assert>
    </sch:rule>
  </sch:pattern>
  <!-- added to odd -->
  <sch:pattern>
    ...
    <sch:rule context="tei:desc[@type = 'definition']">
      <sch:assert test="string-length(.) &lt; 1500" role="error">The description is too long: 1500
        characters maximum. Your description is <sch:value-of select="string-length(.) - 1500"/> too
        long</sch:assert>
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:desc[@type = 'terms']">
      ...
      <sch:report
        ...
        see="http://ssk.huma-num.fr/#/glossary/standards" role="information">A scenario could have
      ...
      <sch:assert test="count(tei:term[@type = 'discipline']) &lt; 5" role="warning">More than 4
        terms of the same vocabulary type may be too much: DISCIPLINE</sch:assert>
      <sch:assert test="count(tei:term[@type = 'technique']) &lt; 5" role="warning">More than 4
        terms of the same vocabulary type may be too much: TECHNIQUE</sch:assert>
  ...
  <sch:pattern>
    ...
    <sch:rule context="tei:desc[@type = 'terms']/tei:term">
      ...
      <sch:report test="ancestor::tei:TEI[@type = 'researchScenario'] and @type = 'activity'"
        role="warning">The activity terms are more suitable for describing steps rather than
        scenarios. It is recommended to choose one activity term per scenario step.</sch:report>
  ...
  <!--<sch:pattern>
    <sch:rule context="tei:linkGrp/tei:ref">
      <sch:assert test="tei:term" role="information">It is possible to add terms to resources, for
        instance a standard or a technique.</sch:assert>
    </sch:rule>
  </sch:pattern>-->
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="tei:event">
      <sch:report
        ...
        role="error">we need a desc element for the description of the step or the scenario and a
        desc element for the associated terms.</sch:report>
